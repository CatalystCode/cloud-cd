...
function execute(config, callback) {
    ...
    ssh_connect(ssh_client, {
        ...
        .then(function () { return ssh_upload(ssh_client, config.script, '/tmp/' + scriptFileName); })
        .then(function () { return ssh_exec(ssh_client, 'sudo bash /tmp/' + scriptFileName + ' ' + (config.args || ''), {
        pty: true
    }); })
